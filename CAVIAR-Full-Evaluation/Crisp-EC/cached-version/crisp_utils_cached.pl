...
recHLE(Timepoint, [Tuple | RestTuples], Stream):-
  ...
  recHLE(Timepoint, RestTuples, Stream). % move on to next tuple

recognize(Fluent = Value, T, Stream):-
  holdsAt(Fluent = Value, T),
  assert(temp_store(Fluent = Value)), 
  (Fluent = person(_) ; writeResult(Fluent, T, Stream)). % don't write person/1 data in ".result" file
...
forget:-
  findall(cached(X), cached(X), L),
  retractAll(L).

store:-
  findall(temp_store(X), temp_store(X), L),
  assertAllCached(L), % asserts all temporaries as cached data
  retractAll(L). % forgets all temporaries
...
assertAllCached([Head | Tail]):-
  Head =..[_Functor, FluentAndValue],
  assert(cached(FluentAndValue)),
